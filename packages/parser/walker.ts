...
const Walker = async (
  fiber: TFiberNode | null | undefined,
  ...
  try {
    ...
    const { walkerOptionsForNextSibling, ...parsedFiber } = await handler(
      fiber,
      Walker
    );
    ...
    if (parsedFiber) {
      if (
        fiber.sibling &&
        walkerOptionsForNextSibling?.escapeSibling !== true &&
        escapeSibling !== true
      ) {
        return [parsedFiber, ...(await Walker(fiber.sibling))];
      }
      return [parsedFiber];
    }
